...
**用途**：检查所有已安装能力包的健康状态——重新验证 anchor 位置是否还有效，contracts 是否还能通过，是否有能力包因为基础代码变更而"静默损坏"。
...
### Phase 4 — Contract 验证（仅 `--contracts` 模式）
...
| 状态 | 含义 | 建议行动 |
|------|------|---------|
| ✓ 健康 | 所有 abort probe 通过 | 无需操作 |
| ⚠ 退化 | warn probe 失败，optional 组件失效 | /scsp-sync 或重新安装 |
